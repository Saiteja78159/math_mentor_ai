...
# #     # Add coefficient 1 to x (catch -x, +x, or start of string)
# #     text = re.sub(r'(?<=^|[+\-])x', '1x', text)
...
#     # Add coefficient 1 to standalone x (handles -x, +x, start of string)
#     text = re.sub(r'(?<=^|[\+\-])x', '1x', text)

#     # Add coefficient 1 for -x at start of string
#     text = re.sub(r'^-x', '-1x', text)
...
def normalize_math(text: str) -> str:
    ...
    # Add coefficient 1 to standalone x (handles +x, -x, start of string)
    # Replace start-of-string x
    text = re.sub(r'^x', '1x', text)
    # Replace +x or -x
    text = re.sub(r'([+\-])x', r'\11x', text)

    # Replace -x at start of string if missed
    text = re.sub(r'^-x', '-1x', text)
